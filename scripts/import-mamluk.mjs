...
const DIR = "E:\\Own\\Clients\\AfghanTappeti\\products\\2-Mamluk-48-cropped"
...
async function main() {
  ...
  let { data: cat } = await supabase.from("categories").select("id").eq("name", "Mamluk").single()
  if (!cat) {
    const id = crypto.randomUUID()
    await supabase.from("categories").insert({ id, name: "Mamluk", slug: "mamluk", display_order: 999, is_active: true })
    cat = { id }
    console.log("Created category: Mamluk")
  }

  // Get or create collection
  let { data: col } = await supabase.from("collections").select("id").eq("name", "Mamluk Collection").single()
  if (!col) {
    const id = crypto.randomUUID()
    await supabase.from("collections").insert({ id, name: "Mamluk Collection", slug: "mamluk-collection", is_active: true })
    col = { id }
  }
  ...
  for (const [num, product] of productMap) {
    const sku = `MLK-${num.padStart(3, "0")}`
    ...
    for (let i = 0; i < product.images.length; i++) {
      ...
      while (retries > 0) {
        ...
        if (!error) {
          await supabase.from("product_images").insert({ product_id: productId, image_url: storageName, alt_text: `Mamluk Rug ${num}`, display_order: i, is_primary: i === 0 })
          imgsUploaded++
          break
        }
        retries--
