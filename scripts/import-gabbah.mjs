...
const DIR = "E:\\Own\\Clients\\AfghanTappeti\\products\\Sultani gabah"
...
async function main() {
  ...
  let { data: cat } = await supabase.from("categories").select("id").eq("name", "Sultani Gabbah").single()
  if (!cat) {
    const id = crypto.randomUUID()
    await supabase.from("categories").insert({ id, name: "Sultani Gabbah", slug: "sultani-gabbah", display_order: 999, is_active: true })
    cat = { id }
    console.log("Created category: Sultani Gabbah")
  }
  ...
  for (const [num, product] of productMap) {
    ...
    for (let i = 0; i < product.images.length; i++) {
      ...
      while (retries > 0) {
        ...
        if (!error) {
          await supabase.from("product_images").insert({ product_id: productId, image_url: storageName, alt_text: `Sultani Gabbah Rug ${num}`, display_order: i, is_primary: i === 0 })
          imgsUploaded++
          break
        }
        retries--
