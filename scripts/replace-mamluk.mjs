...
const FOLDER = "2-Mamluk-48-cropped"
const PREFIX = "MLK"
const FOLDER_PATH = "E:\\Own\\Clients\\AfghanTappeti\\products\\2-Mamluk-48-cropped"
...
async function main() {
  console.log(`=== Convert & Replace: ${FOLDER} ===\n`)
  ...
  for (const [num, product] of productMap) {
    ...
    if (existing) {
      ...
      const { error: prodError } = await supabase.from("products").insert({
        ...
        sku,
        price,
        description: `Hand-knotted Mamluk rug. Size: ${product.width}×${product.height} cm.`,
        stock_quantity: 1,
        is_active: true,
      })
