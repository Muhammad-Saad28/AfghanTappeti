...
export default function CartPage() {
  ...
  const t = dictionaries[locale].cart
  const { cart, removeItem, updateQuantity, clearCart, subtotal } = useCart()

  if (cart.length === 0) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-4">
        <h1 className="font-headline-sm text-headline-sm text-on-surface mb-3">{t.empty}</h1>
        <p className="text-on-surface-variant font-body-md mb-8">Add some rugs to get started.</p>
        <Link href={`/${locale}/shop`} className="bg-primary text-on-primary px-6 py-3 rounded-lg text-label-md no-underline hover:bg-primary-fixed-dim transition-colors">{t.empty_cta}</Link>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen pt-32 pb-16 px-margin-mobile md:px-margin-desktop max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        ...
        <button onClick={clearCart} className="text-label-md text-on-surface-variant hover:text-error transition-colors">{t.clear}</button>
      </div>
      <div className="flex flex-col lg:flex-row gap-12">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-2">
                  <div className="flex items-center border border-outline-variant rounded">
                    ...
                    <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="p-1.5 hover:bg-surface-variant transition-colors"><Plus size={14} /></button>
                  </div>
                  <button onClick={() => removeItem(item.id)} className="text-on-surface-variant hover:text-error transition-colors"><Trash2 size={16} /></button>
                </div>
              </div>
        ...
        <div className="lg:w-80">
          <div className="bg-surface rounded-xl border border-outline-variant p-6 sticky top-32">
            <h2 className="font-headline-xs text-headline-xs text-on-surface mb-4">{t.order_summary}</h2>
            <div className="space-y-3 font-body-md">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-on-surface-variant">{t.shipping_calc}</span>
              </div>
            ...
            <Link href={`/${locale}/shop`} className="flex items-center justify-center gap-1 text-label-sm text-on-surface-variant hover:text-secondary no-underline transition-colors mt-3"><ArrowLeft size={14} /> {t.continue}</Link>
          </div>
        </div>
      </div>
    </div>
